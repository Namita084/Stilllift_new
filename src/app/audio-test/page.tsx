...
export default function AudioTestPage() {
  ...
  const handlePlay = async (testTitle?: string, testMessage?: string, testOptions?: PlayTextAudioOptions) => {
    setIsPlaying(true);
    const currentTitle = testTitle ?? title;
    const currentMessage = testMessage ?? message;
    const currentOptions = testOptions ?? options;
    
    setLastPlayed(`${currentTitle ? currentTitle + ': ' : ''}${currentMessage}`);
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-8">
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="text-center">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">Audio Library Test</h1>
          <p className="text-lg text-gray-600">Test your audio library functionality</p>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Test with your own title and message</CardDescription>
          </CardHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <Input
                  ...
                  max="2"
                  value={options.rate}
                  onChange={(e) => setOptions({...options, rate: parseFloat(e.target.value)})}
                />
              </div>
              <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  onChange={(e) => setOptions({...options, isHomepage: e.target.checked})}
                />
              ...
              {predefinedTests.map((test, index) => (
                <div key={index} className="p-4 border rounded-lg space-y-2">
                  ...
                  <p className="text-sm text-gray-600">
                    <strong>Title:</strong> {test.title || 'None'}<br/>
                  ...
                  <Button
                    size="sm"
                    onClick={() => handlePlay(test.title, test.message, test.options)}
                    disabled={isPlaying}
                    className="w-full"
                  >
                    Test
          ...
          <CardContent>
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center p-2 bg-blue-50 rounded">
                <span className="font-medium">Mood_Good_Context_Still_Audio_1.mp3</span>
                <span className="text-sm text-gray-600">Still/Safe context</span>
              </div>
              <div className="flex justify-between items-center p-2 bg-blue-50 rounded">
                <span className="font-medium">Mood_Good_Context_Move_Audio_1.mp3</span>
                <span className="text-sm text-gray-600">Moving context</span>
              </div>
              <div className="flex justify-between items-center p-2 bg-blue-50 rounded">
                <span className="font-medium">Mood_Good_Context_Focussed_Audio_1.mp3</span>
                <span className="text-sm text-gray-600">Focused context</span>
              </div>
              <div className="flex justify-between items-center p-2 bg-purple-50 rounded">
                <span className="font-medium">treasure_chest_revealtoken.mp3</span>
                <span className="text-sm text-gray-600">Special treasure audio</span>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            <div className="text-sm space-y-1">
              <p><strong>1. Test Audio File Matching:</strong> Try the predefined tests to see which audio files are found</p>
              <p><strong>2. Test TTS Fallback:</strong> Use messages that don't match any audio files to test text-to-speech</p>
              <p><strong>3. Test Options:</strong> Adjust rate, pitch, and volume to hear the differences</p>
              <p><strong>4. Test Mood-Context:</strong> Use different mood and context combinations</p>
              <p><strong>5. Check Console:</strong> Open browser dev tools to see detailed audio loading logs</p>
              <p><strong>6. Audio-Screen Matching:</strong> Audio now matches what's displayed: "ACTION: [message]" format</p>
            </div>
          </CardContent>
        </Card>
      </div>
